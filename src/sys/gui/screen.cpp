...
void realizeVoltage(GtkGLArea *area, gpointer userData){
    ...
    screen->createVAO(screen->voltage1);
    screen->createVAO(screen->voltage2);
    screen->createVAO(screen->voltage3);
    screen->createVAO(screen->voltage4);
}
...
void unrealizeVoltage(GtkGLArea *area, gpointer userData){
    ...
    screen->deleteVAO(screen->voltage1);
    screen->deleteVAO(screen->voltage2);
    screen->deleteVAO(screen->voltage3);
    screen->deleteVAO(screen->voltage4);
...
gboolean renderVoltage(GtkGLArea *area, GdkGLContext *context, gpointer userData){
    ...
    if(static_cast<unsigned int>(currentProgram) == (screen->idShaderVolt) && (screen->idShaderVolt) != 0){
        ...
        if(screen->stateSignal1_){
            screen->voltage1.applyOffset(screen->offset1_, screen->voltDiv_);
            screen->drawVAO(screen->voltage1);
        }
        if(screen->stateSignal2_){
            screen->voltage2.applyOffset(screen->offset2_, screen->voltDiv_);
            screen->drawVAO(screen->voltage2);
        }
        if(screen->stateSignal3_){
            screen->voltage3.applyOffset(screen->offset3_, screen->voltDiv_);
            screen->drawVAO(screen->voltage3);
        }
        if(screen->stateSignal4_){
            screen->voltage4.applyOffset(screen->offset4_, screen->voltDiv_);
            screen->drawVAO(screen->voltage4);
        }
    }
...
static void realizeSpectrum(GtkGLArea *area, gpointer userData){
    ...
    screen->createVAO(screen->voltage1.spectrumSignal);
    screen->createVAO(screen->voltage2.spectrumSignal);
    screen->createVAO(screen->voltage3.spectrumSignal);
    screen->createVAO(screen->voltage4.spectrumSignal);
}
...
static void unrealizeSpectrum(GtkGLArea *area, gpointer userData){
    ...
    screen->deleteVAO(screen->voltage1.spectrumSignal);
    screen->deleteVAO(screen->voltage2.spectrumSignal);
    screen->deleteVAO(screen->voltage3.spectrumSignal);
    screen->deleteVAO(screen->voltage4.spectrumSignal);
...
static gboolean renderSpectrum(GtkGLArea *area, GdkGLContext *context, gpointer userData){
    ...
    if(static_cast<unsigned int>(currentProgram) == (screen->idShaderSpec) && (screen->idShaderSpec) != 0){
        ...
        screen->gridVoltage.draw();
        if(screen->stateSignal1_){
            screen->voltage1.calculateSpectrum();
            screen->drawVAO(screen->voltage1.spectrumSignal);
        }
        if(screen->stateSignal2_){
            screen->voltage2.calculateSpectrum();
            screen->drawVAO(screen->voltage2.spectrumSignal);
        }
        if(screen->stateSignal3_){
            screen->voltage3.calculateSpectrum();
            screen->drawVAO(screen->voltage3.spectrumSignal);
        }
        if(screen->stateSignal4_){
            screen->voltage4.calculateSpectrum();
            screen->drawVAO(screen->voltage4.spectrumSignal);
        }
    }
...
static void activate(GtkApplication* widget, gpointer userData){
    Screen *screen = static_cast<Screen*>(userData);
    screen->window = gtk_application_window_new(widget);
    screen->boxPanels = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 5);
        ...
        screen->boxControl = gtk_box_new(GTK_ORIENTATION_VERTICAL, 1);
            ...
            gtk_widget_set_size_request(led, 50, 50);
            GtkStyleContext *context = gtk_widget_get_style_context(led);
            gtk_style_context_add_class(context, "led-off");
            ...
            screen->lavelVoltDiv = gtk_label_new("Volt/Div");
            ...
            gtk_box_pack_start(GTK_BOX(screen->boxControl), screen->lavelVoltDiv, false, false, 5);
...
Screen::Screen()
    ...
    voltage1(8, SIGNAL_COLOR::RED), voltage2(8, SIGNAL_COLOR::ORANGE),
    voltage3(8, SIGNAL_COLOR::BLUE), voltage4(8, SIGNAL_COLOR::GREEN){
